...
class Perceiver3DEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Patch embeddings:
        self.surf_token_embeds = LevelPatchEmbed(surf_vars, patch_size, embed_dim, history_size=1)
        if not self.level_condition:
            self.atmos_token_embeds = LevelPatchEmbed(
                atmos_vars, patch_size, embed_dim, history_size=1
            )
        else:
            self.atmos_token_embeds = LevelConditioned(
                lambda: LevelPatchEmbed(atmos_vars, patch_size, embed_dim, history_size=1),
